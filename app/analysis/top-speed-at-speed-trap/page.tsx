...
export default function Page() {
  const label = ANALYSIS_PAGES.find(p => p.slug === 'top-speed-at-speed-trap')?.label ?? 'Top Speed At Speed Trap'
  return (
    <div className="container mx-auto max-w-screen-xl px-4 py-4">
      <div className="bg-base-100/30 backdrop-blur-md rounded-xl border border-white/10 shadow-2xl p-6">
        ...
        <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
          {ANALYSIS_PAGES.filter(p => p.slug !== 'top-speed-at-speed-trap').slice(0,8).map(p => (
